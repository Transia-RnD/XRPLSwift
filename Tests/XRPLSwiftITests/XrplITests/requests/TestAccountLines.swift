//
//  TestAccountLines.swift
//
//
//  Created by Denis Angell on 8/20/22.
//

// https://github.com/XRPLF/xrpl.js/blob/main/packages/xrpl/test/integration/requests/accountLines.test.ts

import XCTest
@testable import XRPLSwift

final class TestIAccountLines: RippledITestCase {
    ...
    func testJson() async {
        ...
        let request: AccountLinesRequest = try! AccountLinesRequest(json)
        let response: BaseResponse = try! await self.client.request(r: request).wait() as! BaseResponse<AccountLinesResponse>
    ...
    func testModel() async {
        // create the expectation
        let exp = expectation(description: "base")
        let request: AccountLinesRequest = AccountLinesRequest(
            account: self.wallet.classicAddress,
