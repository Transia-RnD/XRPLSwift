...
final class TestWalletSign: XCTestCase {
    ...
    func testSign() {
        ...
        let tx: rTransaction = try! rTransaction(dict)!
    ...
    func testSignCaseInsensitive() {
        ...
        let tx: rTransaction = try! rTransaction(lowercaseMemoTx)!
    ...
    func testInvalidAlreadySigned() {
        let tx: rTransaction = try! rTransaction(RequestFixtures.sign())!
        let result = try! wallet.sign(transaction: tx, multisign: false)
        let cloneTx: [String: AnyObject] = BinaryCodec.decode(buffer: result.txBlob)
        let nextTx = try! rTransaction(cloneTx)
    ...
    func testSignNoFlags() {
        let tx: rTransaction = try! rTransaction([
            "TransactionType": "Payment",
    ...
    func testSignICLowercase() {
        let icTx: rTransaction = try! rTransaction([
            "TransactionType": "Payment",
            "Account": "r9cZA1mLK5R5Am25ArfXFmqgNwjZgnfk59",
            "Destination": "rQ3PTWGLCbPz8ZCicV5tCX3xuymojTng5r",
            "Amount": [
    ...
    func testSignOrHex() {
        ...
        let icTx2: rTransaction = try! rTransaction([
            "TransactionType": "Payment",
            "Account": "r9cZA1mLK5R5Am25ArfXFmqgNwjZgnfk59",
            "Destination": "rQ3PTWGLCbPz8ZCicV5tCX3xuymojTng5r",
            "Amount": [
