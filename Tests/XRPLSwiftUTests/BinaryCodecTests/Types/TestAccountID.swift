...
final class TestUAccountID: XCTestCase {
    ...
    func testFromValueHex() throws {
        let accountId = try AccountID.from(value: TestUAccountID.HEX_ENCODING)
        XCTAssertEqual(accountId.toJson(), TestUAccountID.BASE58_ENCODING)
    }

    func testFromValueBase58() throws {
        let accountId = try AccountID.from(value: TestUAccountID.BASE58_ENCODING)
        ...
        XCTAssertEqual(accountId.toHex(), TestUAccountID.HEX_ENCODING)
    }
